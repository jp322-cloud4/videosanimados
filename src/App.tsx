...
export default function App() {
  ...
  const [selectedPlanPrice, setSelectedPlanPrice] = useState("R$ 29,90");
  ...
  return (
    <div className="min-h-screen bg-dough-beige text-pizza-charcoal flex flex-col font-sans selection:bg-pizza-red selection:text-white">
      ...
      {/* 11. Footer verbatim */}
      <footer className="bg-pizza-charcoal text-gray-400 py-12 px-4 border-t border-amber-950/20">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
          <div className="space-y-2">
            <div className="font-display font-extrabold text-white text-lg tracking-tight">
              🍕 Vídeos Animados para Pizzarias
            </div>
            <p className="text-xs text-gray-500 max-w-sm leading-relaxed">
              Destaque-se no mercado de deliveries e aumente suas vendas semanais com material de marketing premium focado em alta conversão.
            </p>
          </div>
          
          <div className="flex flex-col items-center md:items-end gap-3">
            <div className="flex items-center gap-1.5 text-xs text-gray-500">
              <Shield className="w-3.5 h-3.5 text-pizza-red" />
              <span>Plataforma de Demonstração Segura e Criptografada</span>
            </div>
            <div className="text-sm font-sans tracking-tight text-gray-400">
              © 2026 Vídeos Animados para Pizzarias. Todos os direitos reservados.
            </div>
          </div>
        </div>
      </footer>
